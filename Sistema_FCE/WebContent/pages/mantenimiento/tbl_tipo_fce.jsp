          ...
          <div class="card">
            <div class="card-header">
            	...
            	<%
            		...
            		if (x){
            	%>
              		<h3 class="card-title">TipoFCE</h3>
              	<%}else{ %>
            ...
            <!-- /.card-header -->
            <div class="card-body">
              <a href="./newEstudianteCandidato.jsp" class="redirect"> <i class="fas fa-plus"></i>Agregar Tipo</a>
              <br><br>
              <table id="Tbl_ec" name="Tbl_ec" class="table table-bordered">
                <thead>
                  ...
                  <tbody>
                <%
                	DT_tipo_fce dta = new DT_tipo_fce();
                  	        		ArrayList<Tbl_tipo_fce> listtipo = new ArrayList<Tbl_tipo_fce>();
                  	        		listtipo = dta.listarTipoFce();
                  	        		
                  	        		int id;
                  	        		String tipo="";
                  	        		String descripcion="";
                  	        		String estado;
                  	        		for(Tbl_tipo_fce ti : listtipo)
                  	        		{
                  	        			tipo=tipo==null?" ":tipo;
                  	        			descripcion=descripcion==null?" ":descripcion;
                  	        			tipo = ti.getTipo();
                  	        			descripcion = ti.getDescripcion();
                  	        			estado = ti.getEstado()==1||ti.getEstado()==2?"ACTIVO":"";
                %>
	                <tr>
	                  <td><%=ti.getId()%></td>
	                  <td><%=ti.getTipo()%></td>
	                  <td><%=ti.getDescripcion()%></td>
	                  <td><%=ti.getEstado() %></td>
	                  <td>
	                  <a onclick="linkEditUser('<%=ti.getId()%>');"><i class="far fa-edit" title="Editar"></i></a>
		                  &nbsp;&nbsp;&nbsp;
	                  <a href="#" onclick="deleteUser('<%=ti.getId()%>');"><i class="far fa-trash-alt" title="Eliminar"></i> </a>
	                  </td>
	                </tr>
	             <%
	        		}   
...
<script>
  function deleteOpciones(id) {
        ...
        {
  		    'type': 'confirm',
  		    'confirmQuestion': 'ï¿½Estï¿½ seguro de eliminarlo?',
  		    'onConfirm': function(e, btn){
  		      e.preventDefault();
       		  window.location.href= "../../SL_tipo_fce?id="+ id;
...
<script>
  ...
  function alertC(type, mensaje, title) {
      ...
      'theme': type,
  		...
  		'content': mensaje,
      'onClose': function() {
      	window.history.pushState({page: "another"}, "tbl_tipo_fce", "/Sistema_FCE/pages/mantenimiento/tbl_tipo_fce.jsp")
    	}
  	});
  }
