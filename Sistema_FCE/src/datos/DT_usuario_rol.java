...
public class DT_usuario_rol 
{
	...
	public ArrayList<Vw_usuario_rol> listUserRol()
	{
		...
		try
		{
			PreparedStatement ps = c.prepareStatement("SELECT * from vw_usuario_rol", 
	...
	public Vw_usuario_rol obtenerUserRol(int idURol)
	{
		...
		try
		{
			PreparedStatement ps = c.prepareStatement("SELECT * from Vw_usuario_rol where id = ?", 
					ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE, 
					ResultSet.HOLD_CURSORS_OVER_COMMIT);
			ps.setInt(1, idURol);
			rsUsuarioRol = ps.executeQuery();
	...
	public boolean existeUsuarioRol(int id_usuario, int id_rol)
	{
		...
		try
		{
			PreparedStatement ps = c.prepareStatement("SELECT * from tbl_rol_usuario WHERE id_usuario = ? AND id_rol = ?", 
					ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE, 
					ResultSet.HOLD_CURSORS_OVER_COMMIT);
